...
public class MainActivity extends Activity {
			...
			public void run() {
				...
				builder.setPositiveButton("本地账号",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int whichButton) {
								Toast.makeText(MainActivity.this, "选择使用本地账号",
				...
				builder.setNeutralButton("拉起账号",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int whichButton) {
								Toast.makeText(MainActivity.this, "选择使用拉起账号",
										Toast.LENGTH_LONG).show();
								if (LANG.equals("java")) {
	...
	public void letUserLogout() {
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				if ("cpp".equals(ModuleManager.LANG)) { // 使用C++调用YSDK,
														// 游戏只需要用一种方式即可
					PlatformTest.logout();
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (mAutoLoginWaitingDlg != null
						&& mAutoLoginWaitingDlg.isShowing()) {
		...
		mModuleListView.setAdapter(new ArrayAdapter<BaseModule>(
			...
			public View getView(int position, View convertView,
				...
				if (item != null) {
					...
					} else if (item.equals("QQ登录")
							&& getPlatform() == ePlatform.WX) {
						view.getBackground().setAlpha(60);
						itemText.setTextColor(0x60000000);
					} else {
